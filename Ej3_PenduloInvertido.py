...
def plot(result, valores_iniciales, parametros):
    ...
    u_opt = np.zeros(N)
    for i in range(N-1):
        u_opt[i] = (K-L_laplaciano) @ x_gorro[i] + sum(alpha_sol[l] * kernel(x_sol, x_gorro, l, i, sigma, parametros) for l in range(N))
        k1 = F(x_gorro[i], parametros) + G(x_gorro[i], parametros) * u_opt[i]
        k2 = F(x_gorro[i] + (h/2)*k1, parametros) + G(x_gorro[i] + (h/2)*k1, parametros) * (u_opt[i] + (h/2))
        k3 = F(x_gorro[i] + (h/2)*k2, parametros) + G(x_gorro[i] + (h/2)*k2, parametros) * (u_opt[i] + (h/2))
        k4 = F(x_gorro[i] + h*k3, parametros) + G(x_gorro[i] + h*k3, parametros) * (u_opt[i] + h)
        x_gorro[i+1] = x_gorro[i] + (h/6)*(k1 + 2*k2 + 2*k3 + k4)
    u_opt[N-1] = (K-L_laplaciano) @ x_gorro[N-1] + sum(alpha_sol[l] * kernel(x_sol, x_gorro, l, N-1, sigma, parametros) for l in range(N))
